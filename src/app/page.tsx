...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    // Load liked listings from local storage if needed, or from a user session
    // For this example, we'll keep it in component state.
  }, []);
  ...
  const currentListing = unseenListings.length > 0 ? unseenListings[0] : null;
  ...
  const resetInteractions = () => {
    ...
    setCurrentIndex(0); // Reset to show first listing again
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-10rem)] p-4 md:p-8 relative overflow-hidden">
      {currentListing ? (
        <ListingCard
          key={currentListing.id} // Important for re-rendering and animations
          listing={currentListing}
          onLike={handleLike}
          onDislike={handleDislike}
          // className="transition-all duration-300 ease-out" // Removed this line
        />
      ) : (
